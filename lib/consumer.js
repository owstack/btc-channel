...
var PrivateKey = require('btccore-lib').PrivateKey;
var PublicKey = require('btccore-lib').PublicKey;
var Address = require('btccore-lib').Address;
...
/**
 * @param {Object} opts
 * @param {string|Network} opts.network - 'livenet/btc' or 'testnet/btc'
 * @param {number} opts.expires - unix timestamp in millis since epoch
 *
 * @param {btccore.PrivateKey=} opts.commitmentKey - key to use when negotiating the channel
 *
 ...
 *
 * @param {btccore.PrivateKey=} opts.fundingKey - key to use for funding the channel
...
function Consumer(opts) {
  ...
  /**
   * @type {btccore.Network}
   * @desc Either 'livenet/btc' or 'testnet/btc'
   */
  this.network = Networks.get(opts.network || 'livenet/btc');
  /**
   * @type number
 ...
 * @return {btccore.Transaction} the updated transaction
 */
